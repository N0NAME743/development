...
HACKING MODE v1.2では、YAKUMO本人の基本状態遷移を引き続きシンプルに定義する。
...
HACKING MODE v1.2の成立に上位形態を必要としない。
...
→ 本書 `YAKUMO HACKING MODE Specification v1.2 Official`
...
HACKING MODE専用のOfficial Masterは、v1.2制定時点でも未指定とする。

将来的に専用Masterを制作する場合は、`YAKUMO_Master_v3.0.png`を本人造形の基準とし、該当するOfficial Costumeの構造を維持した上で、原則として以下のHACKING MODE固有要素のみを反映する。
...
| Previous Version | **YAKUMO HACKING MODE Specification v1.1 Official** |
...
- YAKUMO Official Visual Master参照を`YAKUMO_Master_v2.0.png`から**`YAKUMO_Master_v3.0.png`へ更新**
- Default Costume表記を`C01-B Cyber Pleats Style`から**`C01-B Victorian Gothic Cyber Style`へ更新**（Costume Registry v1.4のC01-B改訂に伴う反映、Costume ID `C01-B`自体は変更なし）
- 本書自身のOfficial Versionを**v1.1からv1.2へ更新**
- MONYの人格・造形・Specification参照（`MONY v2.0 Specification v1.0 Official.md`、`MONY_Master_v2.0.png`、`MONY_Expression_Reference_v1.0.png`）には**変更なし**
- HACKING MODE自体の仕様（HACKING EYE、Expression、Hair Breakdown、Visual Effects、MONYとのHACKING SYNC／Operational State、State Transition、Reference Policyの構成、Image Generation Rule、Avoid、Design Principle）には**変更なし**
- 本改訂はDefault Costumeおよび上位Visual Bible／Masterの参照更新のみを反映するものであり、HACKING MODEの発動条件・演出内容を再定義するものではない
