#!/usr/bin/env python3
"""X (Twitter) OAuth2 Authorization Code + PKCE 再認可ヘルパー（YAKUMO用）。

claude_skill_lab/workspace/x_pipeline/reauthorize.py と同じ仕組み。
YAKUMOは投稿権限（tweet.write）が必要なため、x_pipeline側の読み取り専用
スコープとは別に、tweet.write / offline.access を含むスコープで認可する。

新しいXアプリ（別のclient_id/secret）に切り替える場合は、先に.envの
X_CLIENT_ID / X_CLIENT_SECRETを新しい値へ書き換えてからこのスクリプトを
実行すること（このスクリプトはX_ACCESS_TOKEN / X_REFRESH_TOKENだけを
書き換える。X_CLIENT_ID / X_CLIENT_SECRETはそのまま使う）。

前提: 使用するXアプリのUser authentication settingsで
...
# YAKUMOの投稿処理（app/x/poster.py）が必要とする範囲。
# offline.accessが無いとrefresh_tokenが発行されない。
# コード画像添付（app/common/code_image.py）にmedia.writeが必要になった場合は
# ここへ追加すること（docs/credentials.md 6章参照。未確認事項）。
...
def update_env_value(key: str, value: str) -> None:
    """.env の指定キーだけを書き換える（app/x/poster.pyと同じ方式）。"""
