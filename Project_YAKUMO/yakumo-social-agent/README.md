...
- **コード画像化**: 実装済み（`app/common/code_image.py`）— 投稿案に複数行のコードブロックがあれば自動でPNG画像化し、Discordレビュー・X投稿の両方に添付する。ただし現在のAIプロンプトはコードブロックを生成する指示になっていないため、通常運用では実質未使用（将来の「一点だけ抜粋」プロンプト調整・GitHub実装例紹介機能向けの基盤。`docs/architecture.md` 12章）。実行には`fontconfig` / `fonts-dejavu-core`がシステムに必要
