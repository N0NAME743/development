...
- **Phase 4（Discord Bot）**: 完了 — `discord_daemon.py` を常駐させれば、実際のDiscordで承認/却下/修正ができる（承認・却下・修正（AIでの自動再生成含む）とも動作確認済み）
...
- **コード画像化**: 実装済み（`app/common/code_image.py`）— 投稿案に複数行のコードブロックがあれば自動でPNG画像化し、Discordレビュー・X投稿の両方に添付する。実行には`fontconfig` / `fonts-dejavu-core`がシステムに必要（`docs/architecture.md` 12章）
- **「一点だけ抜粋」プロンプト調整**: 実装済み（`config/prompts/transform_to_yakumo.md`等）— 元ネタの手順・設定を網羅的に要約せず、YAKUMOが一番面白いと感じた一点だけを拾うよう指示。コードならfenced code blockで書くよう指示しており、コード画像化と接続されている。実際の生成品質は実運用での確認待ち（`docs/architecture.md` 13章）
