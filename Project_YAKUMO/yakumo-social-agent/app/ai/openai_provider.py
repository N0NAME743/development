"""Phase 3で使用する実AI接続（OpenAI）。AnthropicProviderと同じプロンプト・同じ入出力契約。

USER ACTION REQUIRED: 使用するには OPENAI_API_KEY が必要（.env）。
...
class OpenAIProvider(AIProvider):
    def __init__(self, model: str = "gpt-5"):
        ...
        if not api_key:
            raise RuntimeError(
                "OPENAI_API_KEY (or AI_API_KEY) not set. "
                "USER ACTION REQUIRED: obtain an OpenAI API key and add it to .env "
                "before using OpenAIProvider. Use MockProvider until then."
            )
